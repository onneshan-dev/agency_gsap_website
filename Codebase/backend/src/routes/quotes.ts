...
router.post('/', requireAdmin, validate(createQuoteSchema), async (req: AuthenticatedRequest, res) => {
  try {
...
router.get('/:id', requireAny, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (error) throw error;
    if (!data) { res.status(404).json({ error: 'Quote not found' }); return; }
...
router.post('/:id/respond', requireClient, async (req: AuthenticatedRequest, res) => {
  try {
