...
router.get('/:projectId/tasks', requireAny, async (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/:projectId/tasks', requireAdmin, validate(createTaskSchema), async (req: AuthenticatedRequest, res) => {
  try {
    const { data, error } = await supabaseAdmin
      .from('tasks')
...
router.patch('/:id', requireAdminOrTeam, async (req: AuthenticatedRequest, res) => {
  try {
    if (req.userRole === 'team_member') {
      ...
      if (!status) { res.status(400).json({ error: 'Only status updates allowed' }); return; }
