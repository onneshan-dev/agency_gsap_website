...
router.post('/:id/confirm', requireClient, async (req, res) => {
    try {
        ...
        if (agreement.client_id !== req.userId) {
            res.status(403).json({ error: 'Access denied' });
            return;
        }
        ...
        const updatedChecklist = agreement.checklist_items.map((item, idx) => ({
            ...item,
        ...
        const schedule = quote?.payment_schedule || [];
        if (schedule.length > 0) {
            const milestones = schedule.map((item, idx) => ({
                project_id: project.id,
                title: item.milestone_title,
                description: item.due_condition,
                phase: 'development',
                sort_order: idx + 1,
                payment_amount: item.amount,
                payment_status: item.amount > 0 ? 'pending' : 'not_applicable',
                status: 'pending',
            }));
        ...
        await notifyAdmins('agreement_confirmed', 'Agreement Confirmed', `Client confirmed agreement for "${project.name}". Project has been created.`, { project_id: project.id, url: `/admin/projects/${project.id}` }, ['in_app', 'email']);
